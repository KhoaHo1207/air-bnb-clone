...
export function ListingReservationForm({
  ...
  if (!canBook) {
    return (
      <div
        id={formAnchorId}
        className="border-ink-200 bg-surface rounded-2xl border p-5"
      >
        <p className="text-ink-700 text-sm">{blockedMessage}</p>
      </div>
    );
  }
  if (!isLoggedIn) {
    return (
      ...
      >
        ...
        <Link
          ...
          className="bg-brand-500 hover:bg-brand-600 mt-3 inline-flex rounded-xl px-4 py-2 text-sm font-semibold text-white transition"
        >
          Sign in to continue
        </Link>
      </div>
  ...
  return (
    <form
      ...
      className="border-ink-200 bg-surface space-y-4 rounded-2xl border p-5"
    >
      ...
      <button
        ...
        className="border-ink-200 hover:border-ink-300 hover:bg-ink-50 focus-visible:ring-brand-300 w-full space-y-3 rounded-2xl border p-3 text-left transition focus-visible:ring-2 focus-visible:outline-none"
        aria-label="Open date picker panel"
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-ink-500 text-[11px] font-semibold tracking-[0.2em] uppercase">
              Check-in
            </p>
          ...
          <div className="bg-ink-200 h-8 w-px" />
      ...
      <div className="border-ink-200 bg-surface-muted space-y-3 rounded-2xl border p-3">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-ink-900 text-xs font-semibold">Guests</p>
        ...
        <div className="border-ink-200 space-y-3 border-t pt-3 text-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-ink-900 font-medium">Adults</p>
              <p className="text-ink-500 text-xs">Ages 13 or above</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                type="button"
                className="border-ink-300 text-ink-700 flex h-8 w-8 items-center justify-center rounded-full border disabled:opacity-40"
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-ink-900 font-medium">Children</p>
              <p className="text-ink-500 text-xs">Ages 2–12</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                type="button"
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-ink-900 font-medium">Infants</p>
              <p className="text-ink-500 text-xs">Under 2</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                type="button"
      ...
      {isCalendarOpen ? (
        <div className="bg-ink-900/30 fixed inset-0 z-30 flex items-center justify-center backdrop-blur-[1px]">
          <div className="border-ink-200 bg-surface w-full max-w-md rounded-3xl border p-4 shadow-xl md:p-5">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-ink-500 text-xs font-semibold tracking-[0.18em] uppercase">
                  Select dates
                </p>
              ...
              <button
                type="button"
                className="border-ink-300 text-ink-700 hover:bg-ink-100 rounded-full border px-3 py-1 text-xs font-medium"
            ...
            <div className="border-ink-100 bg-surface-muted rounded-2xl border px-2 pt-2 pb-3">
              <DateRangePicker
                selected={range}
                onSelect={setRange}
      ...
      <div className="bg-ink-50 text-ink-700 rounded-xl p-3 text-sm">
        <div className="flex items-center justify-between">
          <span>
        ...
        <div className="border-ink-200 text-ink-900 mt-2 flex items-center justify-between border-t pt-2 text-base font-semibold">
          <span>Total</span>
          <span>${total}</span>
        </div>
      </div>
...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="bg-brand-500 hover:bg-brand-600 w-full rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-70"
    >
