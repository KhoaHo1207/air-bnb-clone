  ...
  return (
    <section className="border-ink-200 bg-surface overflow-hidden rounded-3xl border shadow-sm">
      <div className="relative">
        ...
        <div className="absolute right-3 bottom-3 rounded-full bg-black/55 px-3 py-1 text-xs font-medium text-white">
          {activeIndex + 1} / {uniqueImages.length}
        </div>
      </div>

      {canNavigate ? (
        <div className="hide-scrollbar border-ink-200 bg-surface-muted flex gap-2 overflow-x-auto border-t p-3">
          {uniqueImages.map((image, index) => (
            <button
              ...
              className={`relative h-16 w-24 shrink-0 overflow-hidden rounded-lg border transition ${
                activeIndex === index
                  ? "border-brand-400 ring-brand-100 ring-2"
